...
class TransformerNeRF(nn.Module):
    def __init__(self, D=8, W=256, input_ch=3, input_ch_views=3, output_ch=4, skips=[4], 
        ...
        """
        NeRF model with Transformer blocks instead of FC layers
        Args:
            D: number of layers in the network
            W: width of each layer
            input_ch: number of input channels for points
            ...
            use_viewdirs: whether to use view directions as input
            vit_dim: dimension of the ViT features
            transformer_depth: number of transformer blocks to use
        ...
        # First half of the network is standard MLPs
        self.pts_linears = nn.ModuleList(
            [nn.Linear(W, W) if i not in self.skips 
             else nn.Linear(W + input_ch, W) for i in range(D//2)]
        )
        
        # Second half replaced with transformer blocks
        self.transformer_blocks = nn.ModuleList(
            [CrossTransformerBlock(W, vit_dim, num_heads, W//num_heads, W*2, dropout=0.1)
             for _ in range(transformer_depth)]
        )
        
        # Final layers
        self.final_linear = nn.Linear(W, W)
    ...
    def forward(self, x):
        ...
        # If needed, expand context to match batch size
        if batch_size > context.shape[0]:
            # Repeat the context along the batch dimension, ensuring transformers get proper input
            # Detach to avoid backward pass issues
            context = context.expand(batch_size, -1, -1).detach()
        
        # Second half: transformer blocks with image features as context
        for block in self.transformer_blocks:
            h = block(h.unsqueeze(1), context).squeeze(1)
